...
// ---- cartões de crédito ----
// invoice = fatura ABERTA (em centavos); limit = limite total; dias = dia do mês.
const CARDS = [
  { id: 'cc_nu',    brand: 'nubank', nick: 'Nubank Ultravioleta', last4: '4417', limit: 850000,  invoice: 234890, closingDay: 28, dueDay: 7,  bestDay: 29 },
  { id: 'cc_itau',  brand: 'itau',   nick: 'Itaú Click',          last4: '9102', limit: 1200000, invoice: 561240, closingDay: 2,  dueDay: 10, bestDay: 3 },
  { id: 'cc_inter', brand: 'inter',  nick: 'Inter Black',         last4: '3388', limit: 500000,  invoice: 98750,  closingDay: 20, dueDay: 1,  bestDay: 21 },
];

// lançamentos que compõem a fatura aberta de cada cartão (despesas = cents negativo)
const CARD_TX = {
  cc_nu: [
    { t: 'iFood', cat: 'Alimentação', d: '08 jun', cents: -8490, type: 'out' },
    { t: 'Amazon', cat: 'Compras', d: '07 jun', cents: -21990, type: 'out' },
    { t: 'Posto Shell', cat: 'Transporte', d: '06 jun', cents: -18030, type: 'out' },
    { t: 'Drogasil', cat: 'Saúde', d: '05 jun', cents: -6740, type: 'out' },
    { t: 'Mercado Livre', cat: 'Compras', d: '03 jun', cents: -34890, type: 'out' },
    { t: 'Spotify', cat: 'Assinaturas', d: '02 jun', cents: -2190, type: 'out' },
  ],
  cc_itau: [
    { t: 'Passagem aérea', cat: 'Viagem', d: '07 jun', cents: -189000, type: 'out' },
    { t: 'Apple Store', cat: 'Compras', d: '05 jun', cents: -129900, type: 'out' },
    { t: 'Restaurante Fasano', cat: 'Alimentação', d: '04 jun', cents: -42400, type: 'out' },
    { t: 'Zara', cat: 'Compras', d: '02 jun', cents: -29940, type: 'out' },
  ],
  cc_inter: [
    { t: 'Netflix', cat: 'Assinaturas', d: '07 jun', cents: -5590, type: 'out' },
    { t: 'Uber', cat: 'Transporte', d: '06 jun', cents: -3420, type: 'out' },
    { t: 'Padaria', cat: 'Alimentação', d: '05 jun', cents: -2840, type: 'out' },
    { t: 'Farmácia', cat: 'Saúde', d: '03 jun', cents: -4720, type: 'out' },
  ],
};
...
function nextDueLabel(dueDay) {
  // próximo vencimento a partir de hoje (10/06/2026 no protótipo)
  const now = new Date(2026, 5, 10);
  let m = now.getMonth();
  if (now.getDate() > dueDay) m = (m + 1) % 12;
  return `${String(dueDay).padStart(2, '0')} ${MES_PT[m]}`;
}
...
const CC = {
  ...
  creditOpenTotal: () => CC.cards.reduce((a, c) => a + c.invoice, 0),
  creditLimitTotal: () => CC.cards.reduce((a, c) => a + c.limit, 0),
  creditAvailableTotal: () => CC.creditLimitTotal() - CC.creditOpenTotal(),
  _seq: 100,
  ...
  // ---- cartões de crédito ----
  newCardId() { CC._seq += 1; return 'cc' + CC._seq; },
  cardTotals(id) {
    const list = CC.cardTx[id] || [];
    const exp = list.reduce((a, b) => a + Math.abs(b.cents), 0);
    return { exp, count: list.length };
  },
  addCard(data) {
    const id = CC.newCardId();
    CC.cards.push({ id, invoice: 0, ...data });
    CC.cardTx[id] = [];
    return id;
  },
  updateCard(id, patch) {
    const c = CC.cards.find(x => x.id === id);
    if (c) Object.assign(c, patch);
  },
  removeCard(id) {
    const i = CC.cards.findIndex(x => x.id === id);
    if (i >= 0) CC.cards.splice(i, 1);
    delete CC.cardTx[id];
  },
  // lançar um gasto no cartão -> aumenta a fatura aberta
  addCardExpense(cardId, { t, cat, cents }) {
    const c = CC.cards.find(x => x.id === cardId);
    if (!c || cents <= 0) return;
    c.invoice += cents;
    CC.cardTx[cardId] = CC.cardTx[cardId] || [];
    CC.cardTx[cardId].unshift({ t, cat, d: CC.today(), cents: -cents, type: 'out' });
  },
  // pagar fatura -> debita da conta escolhida (e do saldo total) e abate a fatura
  payInvoice(cardId, fromAccId, cents) {
    const card = CC.cards.find(x => x.id === cardId);
    const acc = CC.accounts.find(x => x.id === fromAccId);
    if (!card || !acc || cents <= 0) return 0;
    const pay = Math.min(cents, card.invoice);
    card.invoice -= pay;
    acc.balance -= pay;
    CC.tx[fromAccId] = CC.tx[fromAccId] || [];
    CC.tx[fromAccId].unshift({ t: `Fatura ${card.nick}`, cat: 'Fatura cartão', d: CC.today(), cents: -pay, type: 'out' });
    return pay;
  },
};
...
Object.assign(window, { CC, BankCard, brandOf, fmtBRL, MastercardMark, VisaMark, EloMark, nextDueLabel });
