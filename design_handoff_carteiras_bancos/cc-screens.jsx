...
function MiniStat({ label, value, tone, icon, sub }) {
  const col = tone === 'in' ? '#4ADE80' : tone === 'out' ? '#F87171' : '#60A5FA';
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <span style={{ width: 30, height: 30, borderRadius: 9, display: 'grid', placeItems: 'center', background: tone === 'in' ? 'rgba(34,197,94,0.14)' : tone === 'out' ? 'rgba(239,68,68,0.13)' : 'rgba(59,130,246,0.14)' }}>
      ...
      <span style={{ color: '#64748B', fontSize: 11.5, fontWeight: 600 }}>{sub}</span>
    </div>
  );
}

function HomeScreen({ placement, interaction, accounts, onSelect, onAdd, cardW = 288 }) {
      ...
      <div style={{ display: 'grid', gap: 18 }}>
        ...
        <div style={{ display: 'grid', gap: 12 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
            ...
            <MiniStat label="Crédito" value={fmtBRL(CC.accountTotals('picpay').exp)} tone="out" icon="card" sub="fatura PicPay" />
